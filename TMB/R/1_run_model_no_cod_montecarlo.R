...
compile("Consumption.cpp")
dyn.load(dynlib("Consumption"))
...
for(i in 1:length(ages)){
  ...
  data.list <- list(ny = length(unique(data$year)),
               T_0 = minTemp - shift,
               T_l = maxTemp + shift,
               P = data$cod.maturingbiomass[data$age == age & data$area == area]/10e3,
               B = data$cap.maturingbiomass[data$age == age & data$area == area]/10e3,
               Temp =  temp,
               f = data$EmpC[data$age == age & data$area == area],
               predP = test$cod.maturingbiomass[test$age == age & test$area == area]/10e3,
               predB = test$cap.maturingbiomass[test$age == age & test$area == area]/10e3,
               predTemp = test$meantemp[test$age == age & test$area == area])
  obj     <- MakeADFun(data.list, parameters,  DLL = "Consumption", silent = TRUE,
                       map = list(x = factor(NA)))#logb_alpha=factor(NA)))#NULL) # list(x=factor(NA)))
  opt     <- nlminb(obj$par, obj$fn, obj$gr, control = list(iter.max = 1000, eval.max = 1000))
  rep <- sdreport(obj)
  ss <- summary(rep, p.value = TRUE)[-(1:3),]
  
  parest <-as_tibble(ss[1:4, ])
  parest$par = c("x", "balpha", "bh", "sigma")
  parest$age <- age
  parest$sim = sim
...
(parest.all <- parest.all[,c(7,6,5,1:4)])
library(xtable)
...
results.all[results.all$year == 2013, c("year", "age", "area","f", "C", "seC", "lowC", "highC","sim")]
)
...
#  ggsave("../Consumption_report_no_cod/figs/1_consumption_per_year_and_age_2013_montecarlo.pdf", width = 10, height = 6)
  ggsave("../Figures/montecarlo_simulated_temperatures_consumption_eps0.pdf", device = "pdf", dpi = "retina", 
         width = 10, height = 6)
